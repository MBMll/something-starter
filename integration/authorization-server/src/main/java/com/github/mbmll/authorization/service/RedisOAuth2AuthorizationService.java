...
public class RedisOAuth2AuthorizationService extends JdbcOAuth2AuthorizationService {
	...
	public OAuth2Authorization findByToken(String token, OAuth2TokenType tokenType) {
		if (tokenType == null) {
			...
			OAuth2Authorization oAuth2Authorization = redisTemplate.opsForValue()
				.get(buildKeyOAuth2AuthorizationToken(OAuth2ParameterNames.STATE, token));
			if (oAuth2Authorization != null) {
				return oAuth2Authorization;
			}
			// CODE
			oAuth2Authorization = redisTemplate.opsForValue()
				.get(buildKeyOAuth2AuthorizationToken(OAuth2ParameterNames.CODE, token));
			if (oAuth2Authorization != null) {
				return oAuth2Authorization;
			}
			// ACCESS_TOKEN
			oAuth2Authorization = redisTemplate.opsForValue()
				.get(buildKeyOAuth2AuthorizationToken(OAuth2TokenType.ACCESS_TOKEN.getValue(), token));
			if (oAuth2Authorization != null) {
				return oAuth2Authorization;
			}
			// OidcParameterNames.ID_TOKEN
			oAuth2Authorization = redisTemplate.opsForValue()
				.get(buildKeyOAuth2AuthorizationToken(OidcParameterNames.ID_TOKEN, token));
			if (oAuth2Authorization != null) {
				return oAuth2Authorization;
			}
			// OAuth2TokenType.REFRESH_TOKEN
			oAuth2Authorization = redisTemplate.opsForValue()
				.get(buildKeyOAuth2AuthorizationToken(OAuth2TokenType.REFRESH_TOKEN.getValue(), token));
			if (oAuth2Authorization != null) {
				return oAuth2Authorization;
			}
			// OAuth2ParameterNames.DEVICE_CODE
			oAuth2Authorization = redisTemplate.opsForValue()
				.get(buildKeyOAuth2AuthorizationToken(OAuth2ParameterNames.DEVICE_CODE, token));
			if (oAuth2Authorization != null) {
				return oAuth2Authorization;
			}
			// OAuth2ParameterNames.USER_CODE
			oAuth2Authorization = redisTemplate.opsForValue()
				.get(buildKeyOAuth2AuthorizationToken(OAuth2ParameterNames.USER_CODE, token));
			if (oAuth2Authorization != null) {
				return oAuth2Authorization;
			}
		}
		else {
			var oAuth2Authorization = redisTemplate.opsForValue()
				.get(buildKeyOAuth2AuthorizationToken(tokenType.getValue(), token));
			if (oAuth2Authorization != null) {
				return oAuth2Authorization;
			}
		}
	...
	private void save0(OAuth2Authorization authorization) {
		...
		redisTemplate.opsForValue()
			.set(buildKeyOAuth2AuthorizationId(authorization.getId()), authorization, authorizationTimeout,
					TimeUnit.SECONDS);
		// save by token
	...
	private String findToken(OAuth2Authorization authorization) {
		...
		// ACCESS_TOKEN
		OAuth2Authorization.Token<OAuth2AccessToken> oAuth2AccessToken = authorization
		...
		// OAuth2TokenType.REFRESH_TOKEN
		OAuth2Authorization.Token<OAuth2RefreshToken> oAuth2RefreshToken = authorization
		...
		// OAuth2ParameterNames.DEVICE_CODE
		OAuth2Authorization.Token<OAuth2DeviceCode> oAuth2DeviceCodeToken = authorization
	...
	private String buildKeyOAuth2AuthorizationToken(String tokenType, String token) {
		String prefix = springAuthorizationServerRedisProperties.getPrefix();
		return new StringJoiner(SpringAuthorizationServerRedisProperties.REIDS_KEY_DELIMITER).add(prefix)
			.add(OAUTH2_AUTHORIZATION_TOKEN_TYPE)
			.add(tokenType)
			.add(token)
			.toString();
	}
