...
/**
 * refer to
 * {@link org.springframework.security.oauth2.server.authorization.client.JdbcRegisteredClientRepository}
 *
 * @Author xlc
 * @Description
...
public class Jackson2JsonRedisRegisteredClientSerializer implements RedisSerializer<RegisteredClient> {
	...
	private final JavaType registeredClientEntityType = TypeFactory.defaultInstance()
		.constructType(RegisteredClientEntity.class);
	...
	public RegisteredClient deserialize(byte[] bytes) throws SerializationException {
		...
		try {
			...
			TokenSettings.Builder tokenSettingsBuilder = TokenSettings.withSettings(e.getTokenSettings().getSettings());
