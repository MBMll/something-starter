...
public class FileUtilTest extends TestCase {
    ...
    public void testChunkWrite2File() throws IOException {
        File source = new File("C:\\Users\\admin\\Pictures\\Saved Pictures\\新建文件夹\\20210609_60c084feea5d9.jpg");
        ...
        FileChunkUtil.chunkReadFile(source, chunk, data -> {
            long len = length.getAndAdd(data.length);
            log.info("testChunkReadFile {}, {}", data.length, len);
            futures.add(CompletableFuture.runAsync(() -> {
                try {
                    log.info("testChunkWrite2File {}, {}", data.length, len);
                    FileChunkUtil.chunkWriteFile(dessination, len, data);
                } catch (IOException e) {
                    log.error("error", e);
