...
@Data
@Slf4j
public class ClickWordCaptchaServiceImpl extends CommonCaptchaService<CaptchaDTO, CachedImage> {
    ...
    private Map<String, CachedImage> clickWordMap;
    ...
    public ResponseModel check(CaptchaDTO captchaDTO) {
        ResponseModel r = super.check(captchaDTO);
        if (!validatedReq(r)) {
            return r;
        }
        //取坐标信息
        ...
        try {
            ...
            //aes解密
            pointJson = decrypt(captchaDTO.getPointJson(), point.get(0).getSecretKey());
...
//        base64StrToImage(getImageToBase64Str(backgroundImage), "D:\\点击.png");
    ...
    public CaptchaDTO generate(CaptchaCache cache) throws IOException {
        CachedImage clickWord = RandomUtils.pick(clickWordMap);
        BufferedImage bufferedImage = ImageUtils.getBufferedImage(clickWord);
        return getImageData(bufferedImage);
    }

    @Override
    public CachedImage verify(CaptchaDTO paramter, CaptchaCache cache) throws Exception {
        return null;
    }
